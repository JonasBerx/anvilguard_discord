...
with open('./data/item_db.json') as d:
    item_database = {}
    keys_to_retain = ['itemId', 'name', 'class', 'subclass', 'quality', 'itemLevel', 'requiredLevel', 'icon']
...
async def find_item_in_database(ctx, *args):
    ...
    icon_url = 'https://wow.zamimg.com/images/wow/icons/large/' + req_item['icon'] + '.jpg'
    embed = discord.Embed(
        title=req_item['name'],
        url=wowhead_url,
        description='Item ID: ' + str(req_item['itemId']),color=0x964B00)
    embed.set_thumbnail(url=icon_url)
    embed.add_field(name="Quality", value=str(req_item['quality']), inline=True)
    embed.add_field(name="Required Level", value=str(req_item['requiredLevel']), inline=True)
    embed.add_field(name='\u200b', value='\u200b',inline=True)
    embed.add_field(name="Type", value=str(req_item['class']), inline=True)
    embed.add_field(name="Subtype", value=str(req_item['subclass']), inline=True)
    embed.add_field(name='\u200b', value='\u200b',inline=True)
